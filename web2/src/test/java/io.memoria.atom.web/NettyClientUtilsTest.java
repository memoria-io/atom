...
import static io.memoria.atom.web.NettyServerUtils.stringReply;
import static io.memoria.atom.web.TestUtils.httpClient;
import static io.memoria.atom.web.TestUtils.httpServer;
...
class NettyClientUtilsTest {
  ...
  private static Mono<Void> handle(HttpServerRequest req, HttpServerResponse resp) {
    return stringReply.apply(resp).apply(HttpResponseStatus.OK).apply(pong).then();
  }
  ...
  private static Mono<Void> handlePayload(HttpServerRequest req, HttpServerResponse resp) {
    return req.receive()
              .aggregate()
              .asString()
              .flatMap(ping -> stringReply.apply(resp).apply(HttpResponseStatus.OK).apply(ping + pong).then());
  }
}
